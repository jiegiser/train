...
import org.jiegiser.train.business.domain.DailyTrain;
...
import org.jiegiser.train.business.domain.TrainStation;
import org.jiegiser.train.business.enums.SeatTypeEnum;
import org.jiegiser.train.business.enums.TrainTypeEnum;
import org.jiegiser.train.business.mapper.DailyTrainTicketMapper;
import org.jiegiser.train.business.req.DailyTrainTicketQueryReq;
import org.jiegiser.train.business.req.DailyTrainTicketSaveReq;
...
public class DailyTrainTicketService {
    ...
    /**
     * 缓存击穿: 一个热点的 Key 失效，大量请求直接打到数据库;
     *
     * 解决方案①: 增加定时任务: 每隔 30s,主动刷新缓存,此时缓存时间会变回 60s,可以从 redis 客户端工具查看超时时间
     * 解决方案②: 针对数据库查询,增加分布式锁,100 个请求,只有一个拿到锁,去查数据库,其他 99 个请求都快速失败(或者等待),告诉用户稍后重试
     */

    /**
     * 缓存穿透: 去缓存里取数据，因数据库本身就没数据而造成缓存穿透 -- 配置 spring cache 缓存可以缓存 null 值
     */

    /**
     * 缓存雪崩: 由于短时间内, 大量 Key 同时失效,导致大量请求直接打到数据库;
     */
