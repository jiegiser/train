...
import org.jiegiser.train.business.mapper.cust.SkTokenMapperCust;
import org.jiegiser.train.common.resp.PageResp;
import org.jiegiser.train.common.util.SnowUtil;
import org.jiegiser.train.business.domain.SkToken;
import org.jiegiser.train.business.domain.SkTokenExample;
import org.jiegiser.train.business.mapper.SkTokenMapper;
...
public class SkTokenService {
    ...
    public boolean validSkToken(Date date, String trainCode, Long memberId) {
        ...
        // 需要去掉这段，否则发布生产后，体验多人排队功能时，会因拿不到锁而返回：等待 5 秒，加入 20 人时，只有第 1 次循环能拿到锁
        if (!env.equals("dev")) {
            // 先获取令牌锁，再校验令牌余量，防止机器人抢票，lockKey 就是令牌，用来表示【谁能做什么】的一个凭证
        ...
        if (skTokenCount != null) {
            LOG.info("缓存中有该车次令牌大闸的 key：{}", skTokenCountKey);
            ...
            if (skToken.getCount() <= 0) {
                LOG.info("日期【{}】车次【{}】的令牌余量为 0", DateUtil.formatDate(date), trainCode);
                return false;
            }
